...
black_img = ImageTk.PhotoImage(Image.open("black.png"))
...
def remove_buttons(exit, restart):
    exit.grid_remove()
    restart.grid_remove()
...
def restart_game(exit, restart):
    rock_btn["state"] = "active"
    paper_btn["state"] = "active"
    scissor_btn["state"] = "active"
    final_msg_update("")
    reset_score = 0
    comp_score['text'] = str(reset_score)
    player_score['text'] = str(reset_score)
    comp_choice_img = Label(root, image= black_img, bg = "black")
    comp_choice_img.grid(row = 1, column =1)
    player_choice_img = Label(root, image=black_img, bg = "black")
    player_choice_img.grid(row = 1, column =3)
    remove_buttons(exit, restart)
...
def final_winner():
    ...
    if player_sc == 5:
        final_msg_update("YOU WON THE GAME")
        rock_btn["state"] = "disable"
        paper_btn["state"] = "disable"
        scissor_btn["state"] = "disable"
        exit = Button(root, text = "Exit", command= exit_game, width = 10, height = 1, font = ("comic sans", 20, "bold"),
                         bg = "white", fg = "red")
        exit.grid(row =6, column= 1)
        restart = Button(root, text = "Restart", command= lambda: restart_game(exit, restart), width = 10, height = 1,
                         font = ("comic sans", 20, "bold"), bg = "white", fg = "green")
        restart.grid(row = 6, column = 3)
    elif comp_sc == 5:
        final_msg_update("YOU LOST THE GAME")
        rock_btn["state"] = "disable"
        paper_btn["state"] = "disable"
        scissor_btn["state"] = "disable"
        exit = Button(root, text = "Exit", command= exit_game, width = 10, height = 1, font = ("comic sans", 20, "bold"),
...
final_msg.grid(row = 5, columnspan = 5)
